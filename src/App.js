...
import analyzeImageUrl, { isConfiguredAnalisis } from './azure-image-analysis';
import generateImage, { isConfiguredGeneration } from './azure-image-generation';
...
function App() {
  ...
  var isConfiguredAnalisis_value = isConfiguredAnalisis();
  var isConfiguredGeneration_value = isConfiguredGeneration();
  console.log(isConfiguredAnalisis_value, isConfiguredGeneration_value)
  if (!isConfiguredAnalisis_value || !isConfiguredGeneration_value) {
    return <div><p>Key and/or endponit not configured</p></div>
  } else {
    //console.log(result)
